...
sub bracket {
    ...
    print
	"<center>If you believe this passage does not contain an argument, click the \"Not an argument\" button below. Otherwise place square brackets around the individual statements in this argument:</center>",
	$cgi->startform,
	"<center>",
...
sub display_argument {
    ...
    &display_instructions unless $clean;
...
sub makebox {
    ...
    my $result;
    $result .= "\n<td align=center";
    $result .= " bgcolor=$LEFTPAGECOLOR" if $contents =~ /[1-9$dep]/i;
...
sub plaincell {
    ...
    my $result;
    $result .= "\n<td align=\"center\"";
    $result .= " bgcolor=\"$LEFTPAGECOLOR\"" if $contents =~ /[1-9$ind$IND$dep]/;
    $result .= ">\n";
    $result .= "&nbsp";
    $result .= "\n</td>\n";
...
sub bridgecell {
    ...
    my $result;
    $result .= "\n<td align=center";
    $result .= " bgcolor=\"$LEFTPAGECOLOR\"" if $contents =~ /[1-9]/ and @prevcolorrow[$count];
    $result .= ">\n";
    $result .= "&nbsp";
    $result .= "\n</td>\n";
    return $result;
...
sub display_english {
    ...
    print " (<em>See below for unabridged argument.</em>)" if $abridged;
    print
	"<br>",
	"<font color=$LEFTPAGECOLOR>",
	"<strong>",
...
sub display_instructions {
    print <<END;
<hr>
<font size=-1 color=$LEFTPAGECOLOR><strong>INSTRUCTIONS</strong></font>
<font size=-1>
Use small (tab) buttons in table at right to expand rows to left and right.
