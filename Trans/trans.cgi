...
### prev_chosen maintenance
  ...
  CASE: {
	...
	my $vacuous_quant = "";
	$vacuous_quant = &vacuous_quant($trans,$user_trans);
	if ($vacuous_quant) {
	  $quant_type = "universal";
	  $quant_type = "existential"
	    if $vacuous_quant =~ /\$/;
	  $msg = "<em>Your symbolization</em> &nbsp;&lsquo;<tt>$pretty_user_trans</tt>&rsquo;&nbsp; <em>contains an unnecessary $quant_type quantifier; try deleting it and resubmitting your answer.</em>";
	    $msg .= "\n<p>";
	    $action = "Check answer!";
	    last CASE
	  }
