...
$head_BogusSlash = "<h1>Illegitimate `/'!</h1>";
...
$head_InappropriateTVA ="<h2>Inappropriate truth value assignment or mistaken `/'!<h2>";
...
$head_TAssignedToConclusion = "<h2>`<tt><font size=+1>T</font></tt>' assigned to conclusion!</h2>";
$head_FAssignedToPremise = "<h2>`<tt><font size=+1>F</font></tt>' assigned to premise!</h2>";
...
$head_SlashAassignedtoAtom = "<h1>Statement letter assigned `/'!</h1>";
...
$head_BadSlash = "<h2>`/' not assigned to main logical operator!</h2>";
...
sub msg_NotContingent {
    my ($wff) = @_;
    my $pretty_wff = &prettify($wff);
    return "You have asserted that the WFF <p><center><tt>$pretty_wff</tt></center><p> is contingent.  However, a contingent statement is one that is true in at least one row of the truth table and false in at least one row.  This is not the case for this wff, so it is not contingent.  Please return to the previous page, review the truth table and submit another answer."
}
...
###
sub msg_EmptyATT {
    return "Your truth table contains no rows.  The only time such a truth table can count as complete is when the conclusion of the arguent in question is valid.  That is not the case here. Hence, your truth table is incomplete.  Try again.  Begin adding rows to the truth table on which the conclusion is false."
}
...
sub msg_TooManyTVsInATTRow {
    my ($row_num) = @_;
    return "You have entered too many truth values below the argument in Row $row_num.  There should be exactly one truth value (or slash `/') for each statement letter and connective in the argument.  Delete the extraneous truth values and try again."
}
...
sub msg_ConflictingAssignments2 {
    ...
    return "In Row $row_num, the assignment of the truth value &nbsp;<tt>$tv_of_wff_in_tva_area</tt>&nbsp; to the $wff_type &nbsp;<tt>$wff</tt>&nbsp; in the truth value assignment area (i.e., the area to the left of the vertical bar &nbsp;`<tt>|</tt>'&nbsp;) conflicts with the value of &nbsp;<tt>$wff_tv</tt>&nbsp; that is assigned to one or more of the occurrences of &nbsp;<tt>$wff</tt> in the argument in that row.  Make sure that you have assigned truth values to the $wff_type\s in the truth value assignment area in each row consistently with the assignments they receive in the rest of the row."
}

###
sub msg_ConflictingAssignments1 {
    my ($wff,$row_num) = @_;
    return "The statement letter &nbsp;<tt>$wff</tt>&nbsp; has been assigned both &nbsp;<tt>T</tt>&nbsp; and &nbsp;<tt>F</tt>&nbsp; in Row $row_num.  Make sure that you have assigned truth values to each occurrence of every sentence letter consistently in each row of your abbreviated truth table."
}

###
sub msg_ValidityNotDemonstrated {
    return "Although none of the rows your abbreviated truth table have shown that the argument is invalid (since in none of them are the premises true and the conclusion false), as the truth table stands, it does not demonstrate that the argument is valid either, because there are still more ways to make the conclusion false.  Remember that, to show that the argument is valid, you have to show that, on <i>every</i> way of making the conclusion false, at least one of the premises is false as well (that is, that there is no way to make the conclusion false and the premises true).  <p>So begin adding rows for the remaining ways of making the conclusion false.  If, and only if, you exhaust those possibilities without finding a row with all true premises, you will have shown the argument to be valid."
}
...
sub msg_CorrectATTInvalid {
    ...
    $last_atom_tva_pair =~ s/.*(\w)([TF])$/\&nbsp\;\<tt\>$1\<\/tt\> is assigned \&nbsp\;\<tt\>$2\<\/tt\>,\&nbsp\;/;
    my $first_atom_tva_pairs = $ith_tva;
    $first_atom_tva_pairs =~ s/(.*)\w[TF]$/$1/;
    $first_atom_tva_pairs =~ s/(\w)([TF])/\&nbsp\;\<tt\>$1\<\/tt\> is assigned \&nbsp\;\<tt\>$2\<\/tt\>,/g;
...
sub msg_InconsistentTVA {
    return "You have assigned different truth values to the statement letter `<tt>$_[0]</tt>' in Row $_[1].  Truth values must be assigned uniformly in each row to all occurrences of the same sentence letter.  Correct this and resubmit your answer."
}
...
###
sub msg_MungedTVA {
    return "The truth value assignment in Row $_[0] has become corrupted.  It should look like this: <tt>\"$_[1]\"</tt>.  Yours looks like this: <tt>\"$_[2]\"</tt>.  Repair this and resubmit your answer."
}
...
sub msg_InvalidChar {
    my ($row_num) = @_;
    return "You have typed an invalid character in Row $row_num.  Please delete it.  (Only the truth values `<tt>T</tt>' and `<tt>F</tt>' (upper case), the vertical bar `<tt>|</tt>' and spaces should appear in each row.)"
}
...
sub msg_InvalidATTChar {
    my ($row_num) = @_;
    return "You have typed an invalid character in Row $row_num.  Please return to the previous page and delete it.  (Only the truth values `<tt>T</tt>' and `<tt>F</tt>' (upper case), the vertical bar `<tt>|</tt>', spaces, and possibly a forward slash `<tt>/</tt>' should appear in each row.)"
}
...
sub msg_ATTInvalidButtonPressed {
    return "You pressed the \"Invalid\" button, but you have provided no invalidating truth value assignment -- in none of the rows in your truth table are the premises true and the conclusion false.  Did you perhaps mean to press the \"Valid\" button instead?"
}

###
sub msg_ATTValidButtonPressed {
    return "You pressed the \"Valid\" button, but your abbreviated truth table contains a row in which the premises are true and the conclusion false -- in which case the argument is invalid.  Did you perhaps mean to press the \"Invalid\" button instead?"
}
...
###
sub msg_TooManyTVs {
    return "You have too many truth values in Row $_[0].  Be sure you have a column of truth values (T\'s and F\'s in upper case) entered below every connective and atomic statement that occurs in the argument, but no where else.";
}
###
sub msg_IncorrectTV {
